# Requirements Archive: v1-mercado Mercado — ler o World Exchange

**Archived:** 2026-09-01
**Status:** SHIPPED

For current requirements, see `.planning/workstreams/mercado/REQUIREMENTS.md`.
...
- [x] **DETC-02**: Modo `--mercado` separado — vigiar mercado não degrada nem compete com o modo party (invocação PRÓPRIA, ao lado da party, cada uma no seu processo)
  ...
  >
  > **DEFERIDO:** rodar VÁRIAS instâncias simultâneas ("no futuro quem sabe", palavras dele). Exigiria arquivos de calibração separados por instância — o `README.md` já registra isso como não suportado. Note que as duas instâncias de party existiram no passado e deixaram herança viva: o `AGEN-07` (workstream `tiat`) existe para elas não avisarem em dobro.
  >
