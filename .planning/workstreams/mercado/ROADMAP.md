...
- [x] 04-04-PLAN.md — (wave 3) a margem de craft: `[[receita]]` validado, resolucao de nome que quebra em vez de adivinhar, staleness por componente [ANAL-04]
- [x] 04-05-PLAN.md — (wave 4) o fecho: `vigiar-mercado.bat` e o bloco `[[receita]]` comentado no FIM do `config.toml` (arquivo disputado, task isolada e por ultimo) [DETC-02, ANAL-04]

**Waves:** 1 (04-01 ∥ 04-02) → 2 (04-03) → 3 (04-04) → 4 (04-05). O paralelismo da wave 1 existe porque o modulo de analise e PURO: zero sobreposicao de `files_modified` com o laco.
...
| 4. Modo --mercado, análise e console | 5/5 | In Progress|  |
...
### Phase 5: Assertividade do OCR de nome

**Goal:** Reduzir a perda de linhas por leitura torta de nome, SEM afrouxar as guardas
que impedem itens distintos de fundir.

**A medicao que originou a fase** (2026-09-01, catalogo real do usuario, 24 nomes /
29 palavras distintas): as PALAVRAS repetem muito mais que os nomes — `Hunter's`
aparece em 13 dos 24 nomes, `Sealed` em 9. Logo o discriminador certo NAO e
"quao parecidos sao os nomes", e sim **"esta palavra esta no vocabulario conhecido?"**.

    'Hunteds'    desconhecida  ->  corrige para "Hunter's"  (0,80)
    'St«kings'   desconhecida  ->  corrige para 'Stockings' (0,82)
    'Armor'      CONHECIDA     ->  aceita
    'Weapon'     CONHECIDA     ->  aceita

`Armor` e `Weapon` sao AMBAS conhecidas e por isso nunca se corrigem uma na outra —
e exatamente o par que quebrou a faixa cinzenta em 2026-09-01.

**Limite ja medido:** `Cornmon` (erro classico `m`->`rn`) sai como "palavra nova"
porque `Common` nao esta no vocabulario. A forca do metodo CRESCE COM A COBERTURA;
na PRIMEIRA aparicao de um item novo, palavra torta e palavra nova sao
indistinguiveis.

**Segunda direcao, ja prevista e nunca construida:** `mercado_templates_de_nome`
EXISTE no `calibration.json` e esta `None`, e o `CLAUDE.md` ja traz a doutrina —
preferir `cv2.matchTemplate` sobre recortes de nome cacheados assim que o roster e
conhecido, usando OCR so para popular o cache. Texto de UI pixel-identico casa em
~1.0, e o OCR deixa de importar para item conhecido.

**Terceira direcao REFUTADA antes de comecar:** mais escalas de OCR com votacao NAO
resolve. Nas falhas observadas as duas escalas CONCORDARAM no erro (2x e 3x liam
ambas `St«kings`) — e vies sistematico do glifo, nao ruido aleatorio, e votacao so
resolve ruido.

**Decisao de produto a discutir:** o que fazer quando a palavra e desconhecida E nao
tem vizinha clara.

**Requirements**: LEIT-01 (assertividade), LEIT-02 (falha fechada preservada)

**Restricoes que a fase NAO pode violar:**
- NAO afrouxar o corte de similaridade (0,8947 impede `B-grade` x `C-grade Gemstone`
  a 0,9375 de fundir) nem o piso (0,8837).
- NAO desfazer a trava por DIGITOS (D-03), a trava por PALAVRA (D-09) nem a letra de
  grade na assinatura.
- Nenhuma dependencia nova: `rapidfuzz` NAO entra, a metrica e `difflib` da stdlib.
- Toda mudanca precisa de CONTROLE NEGATIVO sobre os pares que NAO podem mudar de
  veredito — a tabela dos oito ja existe em `tests/test_mercado_catalogo.py`.
